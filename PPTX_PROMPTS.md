...
[슬라이드 5] 화면 구성
- 제목: "3. 화면 구성"
- 화면 레이아웃 image 사용: main-page.jpg
...
- 왼쪽 영역: 기능 설명 불릿 포인트
  ...
  • 회색 표시 = 준비중 (데이터 미수록)
- 실제 화면 이미지:booksehlf.jpg 적절하게 배치
...
- 실제 화면 이미지:library.jpg 적절하게 배치

[슬라이드 8] 주요 기능 — 검색 (Quick Search)
- 제목: "4-3. Quick Search"
- 2열 레이아웃:
  [좌측] Quick Search (상단 검색 바)
    ...
    • 결과 클릭 → 해당 코드 본문으로 이동
    • 첨부파일 내 텍스트도 검색 대상
- 실제 화면 이미지:search-result.jpg 적절하게 배치

[슬라이드 9] 주요 기능 — 검색 (고급 검색)
- 제목: "4-4. 고급 검색"
  [우측] 고급 검색 (Advanced Search)
    ...
    • 아코디언 형태 결과 (접기/펼치기)
- 실제 화면 이미지:advanced-search.jpg 적절하게 배치
...
- 비교 요약 카운트: 추가(+), 수정(~), 삭제(-)
- 실제 화면 이미지:code-compare.jpg 적절하게 배치
