# US Code Navigator - 프레젠테이션 자료 정리

> PPTX 제작을 위한 내용 구성 (슬라이드별 정리)

---

## 슬라이드 1: 표지

- **제목**: US CODE NAVIGATOR
- **부제**: 미국 건축/소방 법규 통합 조회 시스템
- **소속**: 하이테크 1본부
- **날짜**: 2025년
...
### 무엇인가?
- 미국 건축 및 소방 관련 법규(Code)를 **영문 원문 + 한글 해석**으로 동시에 조회할 수 있는 **단일 HTML 웹 애플리케이션**
- 서버 없이 `index.html` 하나로 동작하는 **정적 파일 기반 시스템** (Static SPA)
- 별도 설치 불필요 — 브라우저에서 `index.html`을 열면 즉시 사용 가능

### 개발 배경
- 미국 하이테크 시설(반도체 Fab, 배터리 공장 등) 설계 시 참조해야 하는 법규가 다수
- IBC, NFPA 13, NFPA 72, NFPA 318 등 **24종의 법규**를 통합 관리할 필요
- 영문 원문만으로는 이해가 어려워 **한국어 해설 병기**가 필수

### 대상 사용자
- 건축구조, MEP, 전기, 소방(기계/전기), EHS 분야의 설계 엔지니어

---

## 슬라이드 3: 프로그램 주요 특징

### 1. 서버리스 아키텍처 (Single HTML + JSON)
...
### 2. 이중 언어 지원 (EN/KR)
- 모든 조문(Section)에 **영문 원문(ContentEN)**과 **한글 해석(ContentKR)** 병기
- 챕터 제목, 첨부파일 설명도 EN/KR 이중 제공
- 영문은 파란색 좌측 보더, 한글은 노란색 좌측 보더로 시각 구분

### 3. 이미지 파일 참조 시스템 (Figure & Table)
- **CodeAttachment.json**이 이미지 메타데이터를 관리
- `image/` 폴더에 실제 JPG 파일 저장 (266개)
- 파일명 규칙: `{법규}_{챕터}_{섹션.서브섹션}_{유형}({번호}).jpg`
  - 유형: `F` = Figure(그림), `T` = Table(표)
  - 예시: `IBC_10_1003.3.1_F.jpg` → IBC Chapter 10, Section 1003.3.1의 Figure
  - 예시: `NFPA 13_10_10.2.4.2.1_T(a).jpg` → NFPA 13 Chapter 10, Section 10.2.4.2.1의 Table (a)
- 조문(CodeContent)의 `AttachmentID` 필드가 세미콜론(`;`)으로 연결된 첨부파일 ID를 참조
- 화면에서 **썸네일 가로 스크롤**로 표시되며, 클릭하면 **모달 팝업**으로 확대
- 이미지 로드 실패 시 `.jpg` → `.JPG` 대소문자 자동 전환 후 폴백 아이콘 표시

### 4. 북셸프(Bookshelf) UI
- 홈 화면에 실제 책장 형태의 3D 카드 UI로 법규 목록 표시
- 각 법규를 클릭하면 **책 펼침 팝업**(Book Popup)으로 개요 정보 표시
- 공종별(건축구조/소방기계/소방전기), 기관별(ICC/NFPA) 필터링 지원

### 5. 고급 검색 & 코드 비교
- 키워드 검색: 전체 조문의 영문/한글 텍스트에서 실시간 검색
- 필터링: 법규 유형, 공종, 챕터 단위로 검색 범위 지정
- 코드 비교: 두 법규 버전의 동일 섹션을 나란히 비교 (diff 하이라이팅)

### 6. 반응형 디자인
- Tailwind CSS 기반, 1280px ~ 2560px+ 대형 모니터까지 대응
- 사이드바(w-64 고정) + 메인 콘텐츠 영역 분리
- 4단계 반응형 미디어 쿼리 적용 (XL/2XL/3XL/4XL)
...
## 슬라이드 7: 핵심 데이터 — CodeContent.json 상세

### 역할
- 법규의 **실제 조문 내용**을 저장하는 가장 중요한 테이블 (1,712건)

### 주요 필드

| 필드 | 타입 | 설명 | 예시 |
|------|------|------|------|
| ChapterID | string(FK) | 챕터 참조 | "CH002" |
| Subsection | string/number/null | 세부 조항 번호 | 1.1, "1.1", null |
| OrderKey | string | **정렬 키** | "0003.0307.001" |
| TitleEN / TitleKR | string | 영문/한글 제목 | "Area" / "면적, 구획" |
| ContentEN / ContentKR | string | 영문/한글 조문 내용 | (본문 텍스트) |
| Comment | string/null | 해설/주석 (NOTE 표시) | "ASTM D92..." |
| AttachmentID | string/null | 첨부파일 참조 (세미콜론 구분) | "D001;D002" |
| IndexTags | array | 검색용 키워드 태그 | ["정의", "구역"] |
...
## 슬라이드 8: 핵심 데이터 — CodeAttachment.json (이미지 참조)

### 역할
- 법규 내 **Figure(그림)**와 **Table(표)** 이미지 파일의 메타데이터 관리 (264건)

### 주요 필드

| 필드 | 타입 | 설명 | 예시 |
|------|------|------|------|
| AttachmentID | string(PK) | 첨부파일 ID | "D001" |
| Type | string | 유형 | "T"(Table) / "F"(Figure) |
| FileName | string | 이미지 파일명 (확장자 제외) | "IBC_3_307.1.1_T(1)" |
| Chapter / Section / Subsection | number | 위치 정보 | 3 / 307 / 1.1 |
| AttachTitleEN / AttachTitleKR | string | 제목 (EN/KR) | "MAXIMUM ALLOWABLE..." |
| AttachContentEN / AttachContentKR | string | 이미지 설명/각주 | (표 각주 텍스트) |
...
### ModelCode.json (주요 법규 목록, 24종)
| ID | 약칭 | 설명 | 공종 |
|----|------|------|------|
| MC001 | IBC | International Building Code | 건축구조 |
| MC007 | NFPA 13 | 스프링클러 설비 설치 기준 | 소방기계 |
| MC008 | NFPA 14 | 연결송수관 및 호스시스템 | 소방기계 |
| MC009 | NFPA 20 | 소방용 고정펌프 설치기준 | 소방기계 |
| MC010 | NFPA 72 | 화재경보 및 신호설비 | 소방전기 |
| MC013 | NFPA 318 | 반도체 제조시설 방호기준 | 소방기계 |
| MC024 | NFPA 101 | Life Safety Code | 건축구조 |
| ... | ... | (총 24종) | ... |

---

## 슬라이드 10: index.html 구조 — 전체 레이아웃
...
## 슬라이드 11: index.html 구조 — JavaScript 로직
...
## 슬라이드 12: 데이터 흐름 — 사용자 관점

```
[1] 홈 화면
    공종 필터(건축구조/소방기계/...) 또는 기관 필터(ICC/NFPA)로 법규 선택
         ↓
[2] 법규 열람 (라이브러리)
    좌측: 챕터 → 섹션 트리 네비게이션
    우측: 조문 연속 스크롤 (영문 원문 + 한글 해석 + NOTE + 이미지)
         ↓
[3] 조문 상세
    • Section.Subsection 번호 + 제목 (EN/KR)
    • 키워드 태그 (Index) — 클릭 시 관련 조문 팝업
    • 영문 조문 (파란색 보더)
    • 한글 해석 (노란색 보더)
    • NOTE/Comment (빨간색 보더)
    • Figure/Table 썸네일 → 클릭 시 모달 확대
         ↓
[4] 검색 & 비교
    • 고급 검색: 키워드 + 법규/챕터 필터
    • 코드 비교: 두 버전의 조문을 diff 형태로 비교
```

---

## 슬라이드 13: 유지보수 방법 — 데이터 추가/수정
...
### 첨부파일 조회
```javascript
// CodeContent.AttachmentID → CodeAttachment (세미콜론 split)
